# Deployment of SYGA

This repository puts together all different modules of SYGA and runs them cohesively.
The end result is that the webapp can be accessed at a local port.

You can access through this port for testing or on a VPS, reroute HTTP requests to this port.

A production instance can be seen [here](https://syga.space). The (very incomplete) docs are [here](https://telefonovat.github.io/syga--docs/)

## Individual services

The source code to the webapp, api, and engine can be found at the below repositories

- [Webapp in Vue Typescript](https://github.com/telefonovat/webapp-syga)
- [API in Node JS with Typescript](https://github.com/telefonovat/api-syga)
- [Engine in Python](https://github.com/telefonovat/syga--engine)

## How to run

Clone this repository.

Create an .env file as follows.
**WARNING!!!**: Do not change the port numbers for now. Deployment repository is still quite brittle. It may not work if you do!

~~~
WEBAPP_PORT=8100
API_PORT=8200
ENGINE_PORT=5000
~~~

### How to run locally

Run `docker compose -f docker-compose.yaml -f docker-compose.local.yaml up --build`.

The two file flags need to be in order as above.

### Dev mode (Runs local services. Useful for development)

Clone the different SYGA modules to the root under the names webapp, api and engines.

`git clone git@github.com:telefonovat/webapp-syga.git webapp`

`git clone git@github.com:telefonovat/api-syga.git api`

`git clone git@github.com:telefonovat/syga--engine.git engine`

Run `docker compose up -f docker-compose.yaml --build`.

You can test the frontend, api and engine at the ports you specified.

### Production mode

Run `docker compose -f ./docker-compose.yaml -f docker-compose.prod.yaml up --build`.

Redirect your http requests to the webapp port specified above. Nginx running inside docker compose will handle the proxy-passing.
